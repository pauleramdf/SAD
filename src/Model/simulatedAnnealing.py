...
class Model:
    ...
    def salvarSolucao1(self):
        ...
        self.banco.alimentarBanco(df,"solucao1", False)
    
    def salvarSolucao2(self):
        df = self.banco.solucaoToDF(self.otimizacao)
        self.banco.alimentarBanco(df,"solucao2", False)

    def carregarSolucao1(self):
        df = self.banco.lerTabela('solucao1')
        self.otimizacao = self.banco.solucaoToDict1(df)
        df = self.banco.solucaoToList(df)
        return df

    def carregarSolucao2(self):
        df = self.banco.lerTabela('solucao2')
        self.otimizacao = self.banco.solucaoToDict2(df)
        df = self.banco.solucaoToList(df)
        return df